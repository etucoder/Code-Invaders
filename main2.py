...
class Ship(pygame.sprite.Sprite):
    def __init__(self,x,y,w,h,image_path,damage,hp,speed = 5):
        super().__init__()
        self.x = x
        self.y = y
        self.w = w
    ...
    def shoot(self):
        ...
        if (keys[pygame.K_SPACE] or keys[pygame.K_e] or keys[pygame.K_q]) and self.cooldown <= 0:
            laser = Laser(self.rect.centerx,self.rect.top,5,5,damage=self.damage)
            lasers.append(laser)
    ...
    def check_for_collisions(self):
        ...
        for laser in lasers:
            ...
            if memory_error_alive == False or self.image_path == "memoryerror.png":
                ...
                self.y_speed = 0.75 * self.og_y_speed
            if self.hp <= 0:
                self.kill()
...
level_list = [level1,level2,level3,level4,level5,level6,level7,level8,level9,level10]
...
async def main():
    global files_destroyed,bugsnum,cards_were_shuffled,card_options,card_was_chosen,symbols,current_level,keys,running,files,pro_ships,lasers,level_list,level,startx,starty,rowindex,colindex,spacer,bugs
    while running:
        clock.tick(FPS)
